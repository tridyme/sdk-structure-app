...
Afin de vérifier que Node.js version 16 a bien été installé, vous pouvez utiliser **Invite de Commandes** (cmd.exe sur Windows), et tapez la commande `node -v` afin de vérifier que vous n'avez pas de message d'erreur et que la version de Node est bien égale à la version 16.
...
Afin de permettre l'intégration de l'application dans la marketplace de <a name="https://tridyme.com">TriDyme</a>, nous allons utiliser le module federation, pour cela:
1/ Il faut d'abord activer le **module-federation**:
...
2/ Il faut ensuite nommer la route sur laquelle <a name="https://tridyme.com">TriDyme</a> redirigera pour accéder à l'application.
...
3/ Il faut ensuite modifier le fichier **modulefederation.config.js** au niveau du paramètre "name":

```js
const { dependencies } = require("./package.json");

module.exports = {
  name: "nomDeMonApplication",
  exposes: {
    "./App": "./src/App",
  },
...
4/ Le déploiement en local (lancement sur http://localhost:3000)s'effectue avec la commande:
